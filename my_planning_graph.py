...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for act in self.all_actions:
            pg_act = PgNode_a(act)
            if pg_act.prenodes.issubset(self.s_levels[level]):
                action_set.add(pg_act)
                for s_node in self.s_levels[level]:
                    if s_node in pg_act.prenodes:
                        pg_act.parents.add(s_node)
                        s_node.children.add(pg_act)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for parent_s1 in node_s1.parents:
            for parent_s2 in node_s2.parents:
                if not parent_s1.is_mutex(parent_s2) and not parent_s2.is_mutex(parent_s1):
                    return False
        return True
